...
import { Zap, X, TrendingUp, Activity, Copy, CheckCircle, Lightbulb, CheckCircle2 } from 'lucide-react';
...
function MetricsModal({ propertyType, propertyName, addressCode, onClose }: MetricsModalProps) {
  const weeklyData = [
    { day: 'Mon', consumption: 42.5, cost: 12.75 },
    { day: 'Tue', consumption: 38.2, cost: 11.46 },
    { day: 'Wed', consumption: 45.8, cost: 13.74 },
    { day: 'Thu', consumption: 41.3, cost: 12.39 },
    { day: 'Fri', consumption: 48.6, cost: 14.58 },
    { day: 'Sat', consumption: 52.4, cost: 15.72 },
    { day: 'Sun', consumption: 49.8, cost: 14.94 }
  ];

  const monthlyData = [
    { month: 'Jan', consumption: 324, cost: 97.2 },
    { month: 'Feb', consumption: 298, cost: 89.4 },
    { month: 'Mar', consumption: 315, cost: 94.5 },
    { month: 'Apr', consumption: 342, cost: 102.6 },
    { month: 'May', consumption: 385, cost: 115.5 },
    { month: 'Jun', consumption: 425, cost: 127.5 },
    { month: 'Jul', consumption: 468, cost: 140.4 },
    { month: 'Aug', consumption: 452, cost: 135.6 },
    { month: 'Sep', consumption: 398, cost: 119.4 },
    { month: 'Oct', consumption: 365, cost: 109.5 },
    { month: 'Nov', consumption: 335, cost: 100.5 },
    { month: 'Dec', consumption: 312, cost: 93.6 }
  ];
      ...
      <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-xl border border-white/10 max-w-6xl w-full max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-slate-900/95 backdrop-blur-sm border-b border-white/10 p-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">Electricity Metrics</h2>
        ...
        <div className="p-6 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-6">
              ...
              <p className="text-3xl font-bold text-white">{avgConsumption} kWh</p>
            ...
            <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-6">
              ...
              <p className="text-3xl font-bold text-white">{totalAnnual} kWh</p>
              ...
              {monthlyData.map((data) => (
                <div key={data.month} className="flex items-center gap-4">
                  ...
                  <div className="flex-1 bg-slate-700/30 rounded-full h-8 relative overflow-hidden">
                    <div
                      className="absolute inset-y-0 left-0 bg-gradient-to-r from-amber-500 to-yellow-400 rounded-full transition-all duration-500 flex items-center justify-end pr-3"
                      style={{ width: `${(data.consumption / maxConsumption) * 100}%` }}
                    >
                      <span className="text-white text-xs font-semibold">{data.consumption} kWh</span>
                    </div>
                  </div>
              ...
              {weeklyData.map((data) => (
                <div key={data.day} className="flex-1 flex flex-col items-center gap-2">
                  <div className="relative w-full">
                    <div
                      className="w-full bg-gradient-to-t from-amber-500 to-yellow-400 rounded-t-lg transition-all duration-500 hover:opacity-80"
                      style={{ height: `${(data.consumption / maxWeeklyConsumption) * 200}px` }}
                      title={`${data.day}: ${data.consumption} kWh`}
                    />
                  </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            <Zap className="w-8 h-8 text-amber-400" />
            <h2 className="text-3xl font-bold text-white">Electricity</h2>
          </div>
            ...
            <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-slate-300 text-sm">No problems detected with your electricity service at this time.</p>
                </div>
              </div>
            </div>
          </section>
              ...
              <div className="space-y-4">
                <div className="flex items-start gap-4 p-4 bg-white/5 rounded-lg hover:bg-white/10 transition-colors">
                  ...
                  <div className="flex-1">
                    <h5 className="text-white font-semibold mb-1">Optimize AC usage during peak hours</h5>
                    <p className="text-slate-300 text-sm">Set your thermostat to 74°F between 2-6 PM to reduce consumption during expensive peak hours and save on monthly costs.</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-4 p-4 bg-white/5 rounded-lg hover:bg-white/10 transition-colors">
                  <Lightbulb className="w-6 h-6 text-yellow-400 flex-shrink-0 mt-1" />
                  <div className="flex-1">
                    <h5 className="text-white font-semibold mb-1">Switch to LED bulbs</h5>
                    <p className="text-slate-300 text-sm">Replace incandescent bulbs with LED alternatives to reduce lighting costs by up to 75% and extend bulb lifespan significantly.</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-4 p-4 bg-white/5 rounded-lg hover:bg-white/10 transition-colors">
                  <Lightbulb className="w-6 h-6 text-yellow-400 flex-shrink-0 mt-1" />
                  <div className="flex-1">
                    <h5 className="text-white font-semibold mb-1">Unplug idle electronics</h5>
                    <p className="text-slate-300 text-sm">Devices in standby mode still consume power. Unplug chargers and electronics when not in use or use smart power strips.</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-4 p-4 bg-white/5 rounded-lg hover:bg-white/10 transition-colors">
                  <Lightbulb className="w-6 h-6 text-yellow-400 flex-shrink-0 mt-1" />
                  <div className="flex-1">
                    <h5 className="text-white font-semibold mb-1">Schedule appliance maintenance</h5>
                    <p className="text-slate-300 text-sm">Regular maintenance of HVAC systems, refrigerators, and other major appliances ensures they operate efficiently and consume less energy.</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-4 p-4 bg-white/5 rounded-lg hover:bg-white/10 transition-colors">
                  <Lightbulb className="w-6 h-6 text-yellow-400 flex-shrink-0 mt-1" />
                  <div className="flex-1">
                    <h5 className="text-white font-semibold mb-1">Install smart home energy monitoring</h5>
                    <p className="text-slate-300 text-sm">Use smart plugs and energy monitors to track real-time consumption and identify energy-hungry devices for better management.</p>
                  </div>
                </div>
              </div>
            </div>
